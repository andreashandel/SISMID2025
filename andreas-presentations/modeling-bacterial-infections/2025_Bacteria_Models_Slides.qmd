...
- Take a look at the app, explore the tasks.


## Exploration exercise 2

- Using a pre-written app is user-friendly but not that flexible.
-  We could instead write our own R code that loops over parameters and repeatedly calls the underlying model _('Level 2' of the DSAIRM package tutorial)._
- We could also write our own model and decide to explore some outcome of interest as a function of some parameters _('Level 3' of the DSAIRM package tutorial)._ 
...
* Start an R script. Write code that implements a loop over a parameter of your choice, for each value calls the `simulate_basicbacteria_ode()` model function, and computes some outcome of interest. If needed, you can copy/paste/modify the solution from task 6 you looked at as homework.

**Intermediate:**

* Use the _Extended Bacteria Model_ app and write some R code to loop over 2 (or more) parameters of your choice and plot the results (whatever outcome you consider interesting to explore).

**Advanced:**

* Extend the code of the _Extended Bacteria Model_ (for instance by implementing 2 innate response players, such as NK cells and IFN-gamma). Then explore some outcome of your choice as a function of some of the model parameters.
